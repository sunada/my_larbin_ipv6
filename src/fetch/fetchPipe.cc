...
void checkAll () {
  //printf("in checkAll.\n");
  //printf("global::nb_conn: %d\n", global::nb_conn);
  // read and write what can be
  for (uint i=0; i<global::nb_conn; i++) {
    Connexion *conn = global::connexions+i;
    //printf("conn->state: %d, connectingC: %d, writeC: %d, openC: %d\n", conn->state, connectingC, writeC, openC);

    switch (conn->state) {
    case connectingC:
    case writeC:
      if (global::ansPoll[conn->socket]) {
        //printf("conn->state is connectingC or writeC and global::ansPoll[conn->socket] is true\n");
  ...
  for (uint i=0; i<global::nb_conn; i++) {
    ...
    //printf("global::connexions+i)->state: %d\n", (global::connexions+i)->state);
